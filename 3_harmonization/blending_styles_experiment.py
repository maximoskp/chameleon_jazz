...
file_name = 'experiment_styles_blends.json'
blends = []
...
# show style and substyle types
for k_type in stylesHMM.keys():
    print(k_type + '-----------------')
    for k_subtype in stylesHMM[k_type].keys():
        print(k_subtype)
...
for i in range(len(i1_pieces)):
    ...
    s2 = stylesHMM[i2_type][i2_subtype]
    
    print(30*'-')
    print(s1.piece_name)
    print(i2_type + ' - ' + i2_subtype)
    ...
    # apply HMM
    new_key = 'BL_' + s1.key + '-' + i2_type + '_' + i2_subtype
    ...
    blended_piece[new_key]['appearing_name'] = 'BL_' + s1.piece_name + '-' + i2_type + '_' + i2_subtype
    ...
    plt.savefig(debug_path + '/trans_probs' + s1.key.replace(' ', '_') + '-' + (i2_type + '_' + i2_subtype).replace(' ', '_') + '.png', dpi=500)

    plt.clf()
    plt.imshow(delta, cmap='gray_r')
    plt.savefig(debug_path + '/delta' + s1.key.replace(' ', '_') + '-' + (i2_type + '_' + i2_subtype).replace(' ', '_') + '.png', dpi=500)

    plt.clf()
    plt.imshow(markov, cmap='gray_r')
    plt.savefig(debug_path + '/markov' + s1.key.replace(' ', '_') + '-' + (i2_type + '_' + i2_subtype).replace(' ', '_') + '.png', dpi=500)

    plt.clf()
    plt.imshow(obs, cmap='gray_r')
    plt.savefig(debug_path + '/obs' + s1.key.replace(' ', '_') + '-' + (i2_type + '_' + i2_subtype).replace(' ', '_') + '.png', dpi=500)
# end for
