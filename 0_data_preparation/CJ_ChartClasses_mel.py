...
class Chord(ChameleonContext):
    ...
    def set_tonalities(self, piece_tonality=None, estimated_tonality=None):
        ...
        self.rpcp = np.zeros(12).astype(np.float32)
        self.rpcp[ np.mod(self.relative_root['piece_tonality'] + self.numeric_type, 12) ] = 1
        # get ESTIMATED tonality-relative pitch class set
        self.rpcp_estimated_tonality = np.zeros(12).astype(np.float32)
        self.rpcp_estimated_tonality[ np.mod(self.relative_root['estimated_tonality'] + self.numeric_type, 12) ] = 1
        # get GCT
...
class Measure:
    ...
    def assign_melody_to_chord(self, measure_in):  # Doesn't account for held durations
...
class Section(ChameleonContext):
    ...
    def make_measures(self, section_in): #split by melody or by chord
