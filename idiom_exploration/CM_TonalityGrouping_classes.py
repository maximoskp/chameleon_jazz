...
class Tonality:
    ...
    def print_info(self):
        print('Tonality: ----')
        print('self.midi_tonality: ', self.midi_tonality)
...
class MelodyChordSegment:
    ...
    def print_info(self):
        print('Segment info:                    ')
        print('self.is_constraint:              ', self.is_constraint)
        print('self.user_constraint:            ', self.user_constraint)
        print('self.gct_chord:                  ', self.gct_chord)
        print('self.gct_label:                  ', self.gct_label)
        print('self.midi_chord:                 ', self.midi_chord)
        print('self.gct_rpcp:                   ', self.gct_rpcp)
        print('self.melody:                     ', self.melody)
        print('self.tonality:                   ', self.tonality)
        print('self.melody_midi:                ', self.melody_midi)
        print('self.relative_pcs:               ', self.relative_pcs)
        print('self.relative_pcp:               ', self.relative_pcp)
        print('self.important_melody_midi:      ', self.important_melody_midi)
        print('self.important_relative_pcs:     ', self.important_relative_pcs)
...
class MelodyPhrase:
    ...
    def __init__(self, tonality_in, constraints_in, harmonicRhythm_in, importantNotes_in, melody_in, type_in, level_in, ending_offset, phrase_idx=0):
        ...
        self.melody_chord_segments.append( MelodyChordSegment(tmpGCTconstraints , tmpImportantNotes, tmpNotes, self.tonality ) )
        # # __debug__
        # for s in self.melody_chord_segments:
        #     print('melody_chord_segment: ', s.print_info())
                ...
                if i == 0:
                    ...
                    if i == len( self.melody_chord_segments )-1:
                        ...
                        if not self.melody_chord_segments[i-1].melody:
                            ...
                            # TODO: melody_midi is (sometimes) an integer and it crashes when indexed
                            self.melody_chord_segments[i].melody = m21.note.Note( self.melody_chord_segments[i-1].melody_midi[0] )
                            self.melody_chord_segments[i].melody_midi = self.melody_chord_segments[i-1].melody_midi[0]
                            self.melody_chord_segments[i].relative_pcs = self.melody_chord_segments[i-1].relative_pcs
                            self.melody_chord_segments[i].relative_pcp = np.histogram( self.melody_chord_segments[i].relative_pcs, bins=range(13) )[0]
    ...
    def print_info(self):
        print('MelodyPhrase: ----------------------')
        print('self.tonality:           ', self.tonality)
        print('self.idiom_mode_lab:     ', self.idiom_mode_lab)
        print('self.constraints:        ', self.constraints)
        print('self.harmonicRhythm:     ', self.harmonicRhythm)
        print('self.importantNotes:     ', self.importantNotes)
        print('self.melody:             ', self.melody)
        print('self.type:               ', self.type)
        print('self.level:              ', self.level)
        print('self.index:              ', self.index)
        print('self.chord_offsets:     ', self.chord_offsets)
        print('melody_chord_segments: ')
        for s in self.melody_chord_segments:
            s.print_info()
